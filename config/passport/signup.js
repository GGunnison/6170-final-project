...
module.exports = function (passport) {
  ...
  function (req, email, password, done) {
    if (req.body.passwordConfirm !== password) return done( null, false, req.flash('alert', 'Password fields do not match'));
    ...
    if ( !req.user ) {
      async.parallel({
        ...
        function (err, results) {
          if (results.isStudent || results.isEmployer) return done(null, false, req.flash('alert', 'Email is already taken.') );
          ...
          newUser.save( function (err) {
            if (err) {
              ...
              return done( null, false, req.flash('alert', message) );
            }
